...
<form v-if="showform" @submit.prevent="sendForm()">
    ...
    <div>
        <input :class="{'is-invalid': errors.object}" v-model.trim="object" type="text"  placeholder="Oggetto">
        <p v-for="(error, index) in errors.object" :key="'email'+index" class="error">{{error}}</p>
    </div>
